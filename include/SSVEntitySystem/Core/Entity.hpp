...
private:
    ...
    {
        for(const auto& c : components) c->update(mFT);
    }
    inline void draw()
    {
...
public:
    inline Entity(const EntityStat& mStat, Manager& mManager) noexcept
    : stat(mStat),
      manager(mManager)
    {
    }
    ...
    inline void setDrawPriority(int mDrawPriority)
    {
    ...
    {
        return typeIdsBitset[Impl::getTypeIdx<T>()];
    }
    template <typename T>
    inline T& getComponent() noexcept
    {
        SSVU_ASSERT(hasComponent<T>());
        return ssvu::castUp<T>(*componentPtrs[Impl::getTypeIdx<T>()]);
    }
    template <typename T, typename... TArgs>
    inline T& createComponent(TArgs&&... mArgs)
    {
        SSVU_ASSERT(!hasComponent<T>());

        auto& result(manager.componentRecycler.getCreateEmplace<T>(
        components, *this, FWD(mArgs)...));
    ...
    inline void setGroups(bool mOn, Group mGroup, TGroups... mGroups) noexcept
    {
